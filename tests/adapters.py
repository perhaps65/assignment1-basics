...
from cs336_basics.ch03_transformer.s04_feedforward import SwiGLU, silu
...
from cs336_basics.ch03_transformer.s07_attention import (
    MultiHeadSelfAttention,
    MultiHeadSelfAttentionWithRoPE,
    scaled_dot_product_attention,
)
from cs336_basics.ch03_transformer.s08_transformer_block import TransformerBlock
from cs336_basics.ch03_transformer.s09_transformer_lm import TransformerLM
from cs336_basics.ch04_training.s01_cross_entropy import cross_entropy
from cs336_basics.ch04_training.s03_adamw import AdamW
from cs336_basics.ch04_training.s04_lr_schedule import get_lr_cosine_schedule
from cs336_basics.ch04_training.s05_gradient_clipping import gradient_clipping
from cs336_basics.ch05_training_loop.s01_data import get_batch
from cs336_basics.ch05_training_loop.s02_checkpoint import load_checkpoint, save_checkpoint


# 模型参数名遵循下方文档中的 state_dict 键；RoPE 缓存应注册为非持久 buffer。
...
def run_linear(
    ...
    layer = Linear(d_in, d_out, device=in_features.device, dtype=weights.dtype)
...
def run_swiglu(
    ...
    layer = SwiGLU(d_model, d_ff, device=in_features.device, dtype=w1_weight.dtype)
...
def run_multihead_self_attention(
    ...
    layer = MultiHeadSelfAttention(
        d_model, num_heads, device=in_features.device, dtype=q_proj_weight.dtype,
    )
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    layer = MultiHeadSelfAttentionWithRoPE(
...
def run_transformer_block(
    ...
    layer = TransformerBlock(
        ...
        device=in_features.device, dtype=weights["attn.q_proj.weight"].dtype,
    )
...
def run_rmsnorm(
    ...
    """
    layer = RMSNorm(d_model, eps, device=in_features.device, dtype=weights.dtype)
    layer.load_state_dict({"weight": weights})
    return layer(in_features)
